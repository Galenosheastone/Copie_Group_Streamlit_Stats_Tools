...
# -*- coding: utf-8 -*-
"""
2_UMAP_Streamlit.py (robust 2025-05-29, v1.9)
Streamlit UMAP/XGBoost/SHAP metabolomics app.
...
if uploaded is not None:
    try:
        ...
        y = df[second_col]
        if X.shape[1] < 5:
            st.warning("Fewer than 5 features found. Check your CSV structure!")
        if len(pd.Series(y).unique()) > 10:
            st.warning("More than 10 unique group labels. Are you sure this is a class label?")
...
if do_shap:
    ...
    if arr.ndim == 3:
        if arr.shape[0] == X.shape[0] and arr.shape[1] == X.shape[1]:
            ...
            # (n_classes, n_samples, n_features) alternate
            shap_beeswarm = np.mean(np.abs(arr), axis=0)
        else:
    ...
    with st.expander("Full SHAP beeswarm"):
        try:
            ...
            try:
                ...
                st.warning(f"Unable to plot SHAP violin or beeswarm.")
