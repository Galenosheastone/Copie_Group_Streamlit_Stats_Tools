...
(v1.1 – 2025-05-28, adds custom group-order option)
...
def plot_boxgrid(long_df, mets, palette, pw, show_all, order, fontsize=12, rot=45):
    n, cols = len(mets), 4
    rows = int(np.ceil(n / cols))
    fig, axes = plt.subplots(rows, cols, figsize=(cols*4, rows*4))
    axes = axes.flatten()
    for i, met in enumerate(mets):
        ax, sub = axes[i], long_df[long_df["Metabolite"] == met]
        sns.boxplot(
...
if show_heatmap:
    pivot = (long_df.pivot_table(index="Metabolite", columns="Group",
                                 values="Level")
                    .loc[top_mets, group_order])      # column reorder
...
# ---------- Boxplot grid ----------------------------------------------
bg_fig = plot_boxgrid(long_df, top_mets, palette, pw_dict,
                      show_all, group_order, fontsize, rot)
