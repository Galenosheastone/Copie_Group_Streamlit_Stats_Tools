...
# -*- coding: utf-8 -*-
"""
PLSDA Streamlit App – v2.3 (v1.9 + v2.1 merged + multi-class permutation test)
Author: Galen O'Shea-Stone
Date: 2025-05-30
"""

import streamlit as st
st.set_page_config(page_title="PLSDA Streamlit App v2.3", layout="wide")
...
def perform_permutation_test_with_visualization(
    model, X_train, y_train, n_permutations=1000, method='accuracy'):
    """
    Permutation test for PLSDA: supports binary and multi-class, with accuracy and separation metrics.
    """
    ...
    if method == 'accuracy':
        if len(classes) == 2:
            pred_label = (y_pred > 0.5).astype(int).ravel()
        else:
            # Multi-class: assign predicted class index closest to integer (regression target)
            pred_label = np.argmin(
                np.abs(y_pred.reshape(-1, 1) - np.arange(len(classes))), axis=1
            )
        orig = accuracy_score(y_train, pred_label)
        perm = []
        for _ in range(n_permutations):
            yp = np.random.permutation(y_train)
            m = PLSRegression(n_components=n_comp).fit(X_train, yp)
            pred_perm = m.predict(X_train)
            if len(classes) == 2:
                pred_perm_label = (pred_perm > 0.5).astype(int).ravel()
            else:
                pred_perm_label = np.argmin(
                    np.abs(pred_perm.reshape(-1, 1) - np.arange(len(classes))), axis=1
                )
            perm.append(accuracy_score(yp, pred_perm_label))
        perm = np.array(perm)
        pval = (perm >= orig).sum() + 1
        pval /= (n_permutations + 1)
        fig, ax = plt.subplots(figsize=(10, 6))
        sns.histplot(perm, kde=True, ax=ax)
        ax.axvline(orig, color='red', linestyle='--', label=f'Orig {orig:.3f}')
        ax.set_title(f"Permutation Test – Accuracy ({len(classes)} classes)")
        ax.legend()
        st.pyplot(fig)
        return pval
        ...
        if scores.shape[1] > 1:
            ...
            data = scores[:, [0]]
        centroids = [data[y_train == c].mean(axis=0) for c in classes]
        ...
        for _ in range(n_permutations):
            ...
            cents = [dd[yp == c].mean(axis=0) for c in classes]
            dists_perm = [np.linalg.norm(a - b) for a, b in combinations(cents, 2)]
            perm.append(np.mean(dists_perm))
        perm = np.array(perm)
        pval = (perm >= orig).sum() + 1
        pval /= (n_permutations + 1)
        fig, ax = plt.subplots(figsize=(10, 6))
        sns.histplot(perm, kde=True, ax=ax)
        ax.axvline(orig, color='red', linestyle='--', label=f'Orig {orig:.3f}')
        ax.set_title(f"Permutation Test – Separation ({len(classes)} classes)")
        ax.legend()
        st.pyplot(fig)
        return pval
...
def main():
    ...
    # Multi-class assignment for test set
    if n_classes == 2:
        y_pred_train_bin = (y_pred_train > 0.5).astype(int).ravel()
        y_pred_test_bin  = (y_pred_test  > 0.5).astype(int).ravel()
    else:
        y_pred_train_bin = np.argmin(
            np.abs(y_pred_train.reshape(-1, 1) - np.arange(n_classes)), axis=1)
        y_pred_test_bin = np.argmin(
            np.abs(y_pred_test.reshape(-1, 1) - np.arange(n_classes)), axis=1)
    ...
    for n_comps in range(1, max_comps + 1):
        ...
        # Multi-class assignment for each n_comps
        if n_classes == 2:
            test_pred_binary = np.where(test_pred > 0.5, 1, 0)
        else:
            test_pred_binary = np.argmin(
                np.abs(test_pred.reshape(-1, 1) - np.arange(n_classes)), axis=1)
    ...
    if pval is not None:
        ...
        advice_messages.append("• No valid permutation test result. Cannot assess random-chance performance that way.")
