...
def df_wide_to_long(df: pd.DataFrame) -> pd.DataFrame:
    """Melt wide metabolomics dataframe to long format."""
...
def run_anova(long_df: pd.DataFrame) -> pd.DataFrame:
    """Return dataframe of raw & BH-adjusted p-values."""
    pvals = {}
...
def pairwise_ttests(long_df: pd.DataFrame, mets) -> dict:
    """Return nested dict of pairwise p-values per metabolite."""
...
def plot_heatmap(pivot_df, pw, fontsize=10):
...
def plot_boxgrid(long_df, mets, palette, pw, show_all, fontsize=12):
    n = len(mets)
    cols = 4
    ...
    for i, met in enumerate(mets):
        ax = axes[i]
        sub = long_df[long_df["Metabolite"]==met]
        sns.boxplot(x="Group", y="Level", data=sub, palette=palette,
        ...
        if STATANNOT:
            pairs = [(p.split(" vs ")[0], p.split(" vs ")[1]) for p, pv in pw[met].items()
...
def plot_single(long_df, met, palette, pw, show_all, fontsize=14):
...
with st.expander("ℹ️ Data format & assumptions"):
    st.markdown(
        """
        * **CSV format**: first column = sample ID (any name), second = group/class.
        * Remaining columns: numeric metabolite values.
        * Missing values are ignored.
        * One-way ANOVA per metabolite; p-values BH-FDR adjusted.
        """
    )
...
# Read data and infer columns by position
raw = pd.read_csv(uploaded)
if raw.shape[1] < 3:
    st.error("CSV needs at least 3 columns: ID, Group, and ≥1 metabolite.")
    st.stop()
...
N_top = st.sidebar.slider("# top metabolites", 4, 32, 16, 1)
...
hm_fig = plot_heatmap(pivot, pw_dict, fontsize)
...
# Boxgrid
bg_fig = plot_boxgrid(long_df, top_mets, palette, pw_dict, show_all, fontsize)
...
if choice:
    sm_fig = plot_single(long_df, choice, palette, pw_dict, show_all, fontsize+2)
    st.pyplot(sm_fig)
    st.download_button(f"Download {choice} plot", fig_to_bytes(sm_fig), f"{choice}.png", "image/png")

# Export table
st.markdown("---")
st.download_button("Download significant results (CSV)", sig_df.to_csv().encode(), "anova_sig.csv", "text/csv")
