...
if uploaded_file:
    ...
    if st.button("Run Comparison"):
        ...
        if not results_df.empty:
            ...
            if 'prop_shapiro>0.05' in ranked_df.columns and 'avg_abs_skew' in ranked_df.columns:
                fig, ax = plt.subplots()
                sns.scatterplot(
                    data=ranked_df,
                    x='prop_shapiro>0.05',
                ...
                # Highlight the best with a star
                if not best_transformations.empty:
                    for i, row in best_transformations.iterrows():
                        ax.scatter(
                            row['prop_shapiro>0.05'],
                            row['avg_abs_skew'],
    ...
    if st.button("Process Data"):
        ...
        # --- New: Dynamic file naming for the processed data --- #
        def sanitize_filename(name):
            # Remove spaces and certain special characters for a file-friendly name.
            return name.replace(" ", "_").replace("(", "").replace(")", "").replace("+", "plus").replace("/", "")

        # Use the raw selection values for naming.
        norm_str = normalization_method if normalization_method != "None" else "none"
        log_str = "log" if do_log_transform else "nolog"
        scale_str = scaling_choice if scaling_choice != "None (no scaling)" else "none"

        file_name = f"processed_nmr_data_norm-{sanitize_filename(norm_str)}_log-{log_str}_scale-{sanitize_filename(scale_str)}.csv"
