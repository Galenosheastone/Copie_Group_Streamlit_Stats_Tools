...
# -*- coding: utf-8 -*-
"""
3_Streamlit_PCA_v1.9.py   (bug-fixed)
Created on Thu Jan 30 16:19:17 2025
@author: Galen O'Shea-Stone
"""
import streamlit as st
st.set_page_config(page_title="3_Streamlit_PCA_v1.9", layout="wide")
...
def load_data(file) -> pd.DataFrame:
    ...
    # Down-cast ONLY feature columns to float32 to save RAM
    feature_cols = df.columns[2:]          # first two are sample + group
...
##########################
# ELLIPSE / ELLIPSOID (... unchanged) #
##########################
def hex_to_rgb(hex_color):
    hex_color = hex_color.lstrip('#')
    return tuple(int(hex_color[i:i+2], 16)/255.0 for i in (0, 2, 4))

# ... (plot_confidence_ellipse, make_3d_ellipsoid – unchanged) ...
...
# **Fix:** make group labels plain strings
data[group_col] = data[group_col].astype(str)
data = data.dropna(subset=[group_col])                # safety: remove blank labels
...
pca_df["Group"]    = data[group_col].values        # already str
...
with st.expander("2D PCA Plot", expanded=True):
    ...
    for grp, col in group_color_map.items():
        ...
        ax.scatter(sub["PC1"], sub["PC2"], color=[col], label=str(grp), alpha=0.7)
        if show_ci:
...
if enable_3d and n_components == 3:
    with st.expander("3D PCA Plot"):
        ...
        for grp, col in group_color_map.items():
            sub = pca_df[pca_df["Group"] == grp]
            rgb = f"rgb{tuple(int(c*255) for c in col)}"
            fig3d.add_trace(go.Scatter3d(
                x=sub["PC1"], y=sub["PC2"], z=sub["PC3"],
                mode='markers',
                marker=dict(size=6, color=rgb, opacity=0.7),
                name=str(grp)                            # <- now a safe string
            ))
            if show_ci:
...
# ---------- Loadings heatmap, 2-D / 3-D biplots ----------
# (unchanged – they keep working once the group labels are strings)
# ------------------------------------------------------------------