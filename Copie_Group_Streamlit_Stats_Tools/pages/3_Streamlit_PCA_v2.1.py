...
# Drop any rows lacking a group label
...
with st.expander("2D PCA Biplot"):
    ...
    for i in top_idx:
        vec = loadings[i]
        ax.arrow(0, 0, vec[0]*3*vector_scale, vec[1]*3*vector_scale, color='red', alpha=0.5)
        txt = ax.text(
            vec[0]*3.5*vector_scale, vec[1]*3.5*vector_scale,
...
if enable_3d and n_components == 3:
    with st.expander("Interactive 3D Biplot"):
        loadings = pca.components_.T
        top_idx3 = np.argsort(np.sqrt(loadings[:,0]**2 + loadings[:,1]**2 + loadings[:,2]**2))[-top_n_metabolites:]
        fig_ib = go.Figure()
        for grp, col in group_color_map.items():
            sub = pca_df[pca_df["Group"] == grp]
            rgb = f"rgb{tuple(int(c*255) for c in col)}"
            ...
            if show_ci:
                fig_ib.add_trace(make_3d_ellipsoid(
                    sub["PC1"], sub["PC2"], sub["PC3"], col,
                    name=f"{grp} CI", opacity=ellipsoid_opacity
                ))
        for i in top_idx3:
            vec = loadings[i]
            fig_ib.add_trace(go.Scatter3d(
