...
# ✨ NEW: overview for users  ─────────────────────────────────────────────
with st.expander("ℹ️ About this analysis / outputs", expanded=True):
    st.markdown("""
**What this app does**

1. **Random-Forest feature selection**  
   • Trains a Random Forest on your class labels to rank metabolite importance.  
   • Keeps only the most informative features (based on built-in threshold).

2. **PLS-DA on selected features**  
   • Fits a Partial Least Squares–Discriminant Analysis model (2 components).  
   • Works for **binary or multi-class** data; multi-class handled one-vs-rest.  
   • Generates scores plot to visualise class separation.

3. **Metrics & validation**  
   * Accuracy, R² (train/test), Q² (5-fold), permutation-test *p*-value  
   * Confusion matrix & ROC curves (macro & per-class AUCs)

4. **Variable-importance outputs**  
   * VIP scores table (downloadable CSV)  
   * VIP lollipop plot + heat-map of group means

**Files you can download**

*   `*_vip_table_YYYYMMDD.csv` – all VIP scores  
*   `*_metrics_YYYYMMDD.txt` – classification report  
*   `*_params_YYYYMMDD.txt` – run parameters for reproducibility  
*   `*_confusion_YYYYMMDD.csv` – confusion-matrix counts  
*   PNGs of every figure

Change any sidebar parameter and the cached analysis will refresh automatically.
    """)
# ────────────────────────────────────────────────────────────────────────
