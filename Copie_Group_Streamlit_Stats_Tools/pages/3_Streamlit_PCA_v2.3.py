...
# -*- coding: utf-8 -*-
"""
3_Streamlit_PCA_v2.1.py  – pipeline-aligned version
Created on 2025-05-01 • Updated 2025-05-21
Author: Galen O'Shea-Stone 

This version replicates the matrix MetaboAnalyst feeds to
prcomp(center = TRUE, scale. = FALSE) **without doing any extra
normalisation, transformation, or autoscaling**.
"""
...
def load_data(file) -> pd.DataFrame:
    """
    Read the uploaded CSV and return a clean, *double-precision* dataframe
    that mirrors what MetaboAnalyst passes to `prcomp`.

    • Keeps the first two columns (Sample ID + Group) untouched.
    • Casts feature columns to float64.
    • Drops constant-variance features and all-NA columns.
    • Drops samples that still contain any NA in the feature block.
    • Returns rows sorted by Sample ID for reproducibility.
    """
    df = pd.read_csv(file, encoding_errors="replace", on_bad_lines="skip")

    # numeric cast (float64 by default)
    feature_cols = df.columns[2:]
    df[feature_cols] = df[feature_cols].apply(
        pd.to_numeric, errors="coerce"
    )  # float64

    # remove empty or zero-variance feature columns
    df = df.dropna(axis=1, how="all")                         # all-NA cols
    feature_cols = df.columns[2:]
    zero_var = df[feature_cols].std(ddof=1) == 0
    df = df.drop(columns=feature_cols[zero_var])

    # drop samples that still contain NAs in the feature block
    df = df.dropna(subset=df.columns[2:])

    # stable row order (optional but prevents accidental re-ordering)
    df = df.sort_values(df.columns[0]).reset_index(drop=True)
    return df

@st.cache_resource(show_spinner=False)
def get_trained_pca(X: np.ndarray, n_components: int) -> PCA:
    """Fit PCA in full-SVD mode (same as prcomp)"""
...
ellipsoid_alpha   = st.sidebar.slider("3-D CI opacity", 0.05, 1.0, 0.15)
...
data            = data.dropna(subset=[group_col])           # keep only labelled samples

X      = data.drop([sample_col, group_col], axis=1).values.astype(np.float64)  # float64, no scaling
...
for grp, hexc in zip(groups, default_hex):
    ...
    chosen_hex = st.sidebar.color_picker(grp, st.session_state[state_key], key=f"cp_{grp}")
...
with st.expander("2-D PCA biplot"):
    ...
    for i in idx:
        ...
        t = ax.text(vec[0]*1.15, vec[1]*1.15, data.columns[2:][i],
