#!/usr/bin/env python3
"""
Streamlit app – Data-driven pair-wise tests & visualisation (v2.6, 2025-05-20)
...
def overview_png(
    ...
    if show_legend:
        ...
        fig.legend(handles, list(palette.keys()), loc="lower center", bbox_to_anchor=(0.5,-0.02),ncol=len(palette),fontsize=legend_fs,frameon=False)
...
with st.form("analysis"):
    ...
    choice = st.sidebar.radio("Statistical test",
                ["Student t-test (equal var)","Welch t-test (unequal var)","Mann-Whitney"],index=1)
    if choice.startswith("Student"): test_key="t-equal"
    elif choice.startswith("Welch"): test_key="t"
    else: test_key="MW"
...
if run and upload:
    ...
    st.session_state.update({
        "analysis_ready":True,
        "m":m,"Sig":Sig,"flags":flags,"shap_df":shap_df,
        "groups":m["Group"].cat.categories,
        "test_key":test_key
    })
...
# Retrieve state
m=st.session_state["m"]; Sig=st.session_state["Sig"]
flags=st.session_state["flags"]; shap_df=st.session_state["shap_df"]
groups=st.session_state["groups"]; test_key=st.session_state["test_key"]
...
palette={g:st.sidebar.color_picker(str(g),st.session_state.get(f"color_{g}",default_palette[i%len(default_palette)]),key=f"color_{g}") for i,g in enumerate(groups)}
...
if n_sig:
    ...
    png=overview_png(m,Sig,palette,rows,cols,fs,annot,legend_fs,show_legend,show_xlabels)
    st.image(png,use_container_width=True)
    st.download_button("Download overview PNG",png,"overview.png","image/png")
else: st.info("No metabolites pass the FDR threshold.")
...
with st.expander("🔍 Normality diagnostics"):
    ...
    sel=st.selectbox("Inspect distribution",sorted(m["Metabolite"].unique()))
    for g in groups:
        vals=m[(m["Metabolite"]==sel)&(m["Group"]==g)]["Level"].dropna()
...
sel=st.selectbox("Single-metabolite view",sorted(m["Metabolite"].unique()))
sub=m[m["Metabolite"]==sel]
fig,ax=plt.subplots(figsize=(5,5))
seaborn.boxplot(x="Group",y="Level",data=sub,ax=ax,palette=palette,width=.65,linewidth=2)
seaborn.stripplot(x="Group",y="Level",data=sub,ax=ax,color="black",size=6,jitter=.25)
Annotator(ax,pairs,data=sub,x="Group",y="Level").configure(test=annot,text_format="star",loc="inside",fontsize=fs,verbose=0).apply_and_annotate()
ax.set_title(sel,fontsize=fs+2);ax.set_xticklabels(ax.get_xticklabels(),rotation=15)
plt.tight_layout(); st.pyplot(fig)
buf=io.BytesIO();fig.savefig(buf,dpi=300,bbox_inches="tight")
st.download_button("Download PNG",buf.getvalue(),f"{_sanitize(sel)}.png","image/png")

# Rerun
st.sidebar.divider()
if st.sidebar.button("↺ Re-run analysis"):
    st.session_state["analysis_ready"]=False; st.rerun()
