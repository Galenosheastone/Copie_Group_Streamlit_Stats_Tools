...
def pw_and_fdr_global(
    ...
    """
    Compute one p-value per metabolite (omnibus) then global BH-FDR.
    Returns raw p-values dict and dict of significant metabolites{met: fdr_p}.
    """
    mets, pvals = [], []
    for met, sub in m.groupby("Metabolite"):
        ...
        pvals.append(p)
    # global BH-FDR
    non_nan = [pv for pv in pvals if not np.isnan(pv)]
    _, fdr, _, _ = multipletests(non_nan, method="fdr_bh")
...
def overview_png(
    ...
    for ax, met in zip(axes, mets):
        ...
        seaborn.boxplot(x="Group", y="Level", data=sub,
                        ax=ax, palette=palette, width=.65, linewidth=2)
        seaborn.stripplot(x="Group", y="Level", data=sub,
    ...
    if show_legend:
        handles = [plt.Line2D([], [], marker="s", linestyle="",
            markersize=10, markerfacecolor=palette[g], markeredgecolor="k")
            for g in palette]
...
with st.form("analysis"):
    ...
    choice = st.sidebar.radio(
        "Statistical test",
        ["Student t-test (equal var)", "Welch t-test (unequal var)", "Mann-Whitney"],
        index=1
    )
...
if run and upload:
    ...
    # keep only metabolites passing global FDR
...
with st.expander("🔍 Normality diagnostics"):
    ...
    st.dataframe(
        shap_df.pivot(index="Metabolite",
                      columns="Group",
...
seaborn.boxplot(x="Group", y="Level", data=sub,
                ax=ax, palette=palette, width=.65, linewidth=2)
seaborn.stripplot(x="Group", y="Level", data=sub,
                  ax=ax, color="black", size=6, jitter=.25)
Annotator(ax, pairs, data=sub, x="Group", y="Level") \
    .configure(test=anttation_name, text_format="star",
               loc="inside", fontsize=fs,
