...
def perform_permutation_test_with_visualization(model, X_train, y_train, n_permutations=1000):
    """
    Permutation test based on re-fitting the model with permuted labels 
    and measuring training accuracy. Only suitable for binary classification.
    """
    unique_classes = np.unique(y_train)
    if len(unique_classes) != 2:
        st.warning("Permutation test for accuracy is only implemented for a binary classification (2 groups).")
        return None

    # Obtain the original training accuracy
    ...
    # Retrieve the number of components from the existing (fitted) model
    n_comp = model.n_components

    # Permute labels and measure accuracy distribution
    ...
    # Compute one-tailed p-value: fraction of permuted accuracies >= original_acc
...
def main():
    ...
    # Permutation Test (based on training accuracy)
    st.subheader("Permutation Test for Model Validation (Training Accuracy)")
    p_value = perform_permutation_test_with_visualization(
        plsda, X_train_scaled, y_train, n_permutations=1000
    )
    ...
    if p_value is not None:
        if p_value < 0.05:
            advice_messages.append(
                "• Permutation test (training accuracy) suggests the model is "
                "likely better than random chance (p < 0.05)."
            )
        else:
            advice_messages.append(
                "• Permutation test (training accuracy) suggests the model "
                "may not be significantly different from random (p ≥ 0.05)."
            )
    else:
        advice_messages.append(
            "• No valid permutation test result for multi-class. "
            "Cannot assess random-chance performance that way."
        )
    ...
    if acc_diff > 0.1:
        advice_messages.append(
            f"• Training accuracy ({train_acc:.3f}) is more than 0.1 above test accuracy "
            f"({test_acc:.3f}), indicating potential overfitting."
        )
    else:
        advice_messages.append(
            f"• Training accuracy ({train_acc:.3f}) and test accuracy ({test_acc:.3f}) "
            f"are reasonably close, suggesting less risk of overfitting."
        )

    # 3) Q2 on test set
    if q2_test < 0:
        advice_messages.append(
            "• Negative Q2 on the test set indicates poor predictive performance, "
            "a sign of overfitting or that the model may not generalize well."
        )
    else:
        advice_messages.append(
            f"• Q2 on test set is {q2_test:.3f}, which is >= 0. "
            "This usually indicates some level of predictive ability."
        )
