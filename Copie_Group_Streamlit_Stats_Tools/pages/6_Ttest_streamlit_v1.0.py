#!/usr/bin/env python3
"""
Streamlit app – Pair‑wise t‑tests & visualisation for NMR metabolomics
====================================================================
...
* All calculations run client‑side when possible (no server state between runs).
"""
...
def multi_panel_plot(melted: pd.DataFrame, sig: dict[str, dict[str, float]],
    ...
    if not sig:
        st.info("No metabolites meet the significance threshold (FDR < 0.05).")
        return
    ...
    # Flatten axes for easy iteration; hide unused.
    ...
    for ax in axes[n:]:  # hide any spare axes
        ax.axis("off")
...
st.title("🧪 Metabolomics t‑tests & Visualisation")
...
with st.expander("ℹ️ Required data format", expanded=False):
    st.markdown(
        ...
        "* **Columns 3 → n** – metabolite intensities (numeric, one column per metabolite)"
    )
...
if df.shape[1] < 3:
    st.error("The dataset must contain at least ID, Group, and one metabolite column.")
    st.stop()
