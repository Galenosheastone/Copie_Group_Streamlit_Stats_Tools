...
def pairwise_ttests(long_df: pd.DataFrame, mets) -> dict:
    groups = long_df["Group"].astype(str).unique()
    results = {}
    for met in mets:
        sub = long_df[long_df["Metabolite"] == met]
        res = {}
...
for i, g in enumerate(groups):
    g_str = str(g)
    palette[g_str] = st.sidebar.color_picker(
        g_str,
        mcolors.to_hex(base[i])
    )
