...
def plot_confidence_ellipse(ax, pc1, pc2, color, fill_alpha=0.2):
    """Plot a 95% confidence ellipse for given 2D points with a shaded fill.
    
    Parameters:
      ax: matplotlib Axes
      pc1, pc2: arrays of PCA coordinates
      color: RGB tuple (values between 0 and 1) for the group color
      fill_alpha: Alpha (opacity) value for the ellipse fill (0=transparent, 1=opaque)
    """
    ...
    # Create RGBA colors: fully opaque edge and translucent fill.
    edge_rgba = (color[0], color[1], color[2], 1.0)
    fill_rgba = (color[0], color[1], color[2], fill_alpha)
    ...
    ellipse.set_zorder(0)  # Ensure the ellipse is drawn behind the points.
...
if uploaded_file:
    ...
    ci_fill_alpha = st.sidebar.slider("2D CI Fill Opacity", 0.0, 1.0, 0.2)  # New slider for 2D CI fill transparency

    # Slider for metabolite vector scaling
    vector_scale = st.sidebar.slider("Metabolite Vector Scale", 0.1, 5.0, 1.0)

    # Provide a list of color palette options for default colors
    color_palettes = [
        "husl", "Set1", "Set2", "Dark2", "Paired", "Pastel1", "Pastel2", "Accent", "tab10", "tab20"
    ]
    selected_palette = st.sidebar.selectbox("Select Color Palette for Default Colors", color_palettes, index=0)

    # Generate default group colors from the chosen palette
    default_group_colors = sns.color_palette(selected_palette, len(groups))

    # Sidebar color pickers for each group
    st.sidebar.subheader("Choose Colors for Each Group")
    group_color_map = {}
    for i, group in enumerate(groups):
        default_hex = mcolors.to_hex(default_group_colors[i])
        chosen_color = st.sidebar.color_picker(f"Color for {group}", default_hex)
        group_color_map[group] = hex_to_rgb(chosen_color)
    ...
    for group, color in group_color_map.items():
        ...
        if show_ci:
            plot_confidence_ellipse(ax, subset['PC1'], subset['PC2'], color, fill_alpha=ci_fill_alpha)
