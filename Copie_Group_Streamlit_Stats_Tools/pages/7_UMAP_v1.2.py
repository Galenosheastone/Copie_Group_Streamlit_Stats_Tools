...
# -*- coding: utf-8 -*-
"""
2_UMAP_Streamlit.py (robust-fix 2025-05-27, v1.5 complete)
...
for g in groups:
    ...
    ax2d.scatter(sel["UMAP1"], sel["UMAP2"], label=g, color=col_map[g], alpha=0.7, s=60)
    conf_ellipse(sel["UMAP1"], sel["UMAP2"], ax2d, facecolor=col_map[g], alpha=0.15, edgecolor='black')
ax2d.set(xlabel="UMAP1", ylabel="UMAP2", title="UMAP (2-D)")
ax2d.legend(title="Group"); ax2d.grid()
st.pyplot(fig2d)
...
# ─────────────────────────── 8. Top features ───────────────────────────
corr_matrix = np.corrcoef(Xs.T, u2.T)
corr = corr_matrix[:Xs.shape[1], Xs.shape[1]:]
abs_sum = np.abs(corr).sum(axis=1)
top_idx = np.argsort(abs_sum)[-15:]
rows = [(X.columns[i], abs_sum[i]) for i in top_idx]
...
if do_shap:
    explainer, shap_vals = get_shap_vals(model, pd.DataFrame(X.values, columns=X.columns))
    # aggregate multi-class SHAP
    if isinstance(shap_vals, list):
        shap_arr = np.mean(np.abs(shap_vals), axis=0)
    else:
        shap_arr = shap_vals
    mean_shap = np.abs(shap_arr).mean(axis=0)
    df_shap = pd.DataFrame({"Feature": X.columns, "Mean|SHAP|": mean_shap.tolist()})
    df_shap_top = df_shap.sort_values("Mean|SHAP|", ascending=False).head(20)
    st.subheader("🔎 SHAP Feature Importance (top 20)")
    fig_bar, ax_bar = plt.subplots(figsize=(6,5))
    ax_bar.barh(df_shap_top["Feature"].tolist(), df_shap_top["Mean|SHAP|"].tolist())
    ...
    with st.expander("Full SHAP beeswarm"):
        try:
            shap.summary_plot(shap_arr, pd.DataFrame(X.values, columns=X.columns), feature_names=X.columns, show=False)
            st.pyplot(bbox_inches="tight")
            plt.savefig(os.path.join(OUTPUT_DIRS["shap_plots"], "shap_beeswarm.png"), dpi=600)
        except Exception as e:
            st.warning(f"SHAP beeswarm failed: {e}")
            df_violin = pd.DataFrame({c: shap_arr[:, i].tolist() for i, c in enumerate(X.columns)})
            fig_violin, ax_violin = plt.subplots(figsize=(6,5))
            sns.violinplot(data=df_violin, inner="quartile", ax=ax_violin)
            ax_violin.set_xticklabels(X.columns, rotation=90)
            plt.tight_layout(); st.pyplot(fig_violin)
...
axs[0,0].axis('off')
tbl = axs[0,0].table(cellText=metrics_tbl, colLabels=["Metric","Value"], loc='center')
...
# silhouette plot
samps = silhouette_samples(u2, y_enc)
y0 = 10
for i in np.unique(y_enc):
    group_sil = samps[y_enc==i]; group_sil.sort()
    y1 = y0 + len(group_sil)
    color = plt.cm.nipy_spectral(i/len(groups))
    axs[1,0].fill_betweenx(np.arange(y0, y1), 0, group_sil, facecolor=color, alpha=0.7)
    axs[1,0].text(-0.05, y0 + 0.5*len(group_sil), str(i))
    y0 = y1 + 10
...
f1  = [cr[c]['f1-score'] for c in classes]
...
with st.expander("⬇️ Download key files"):
    ...
    download_button("Confusion matrix CSV", pd.DataFrame(cm.tolist()).to_csv(index=False).encode(), "confusion_matrix.csv")
