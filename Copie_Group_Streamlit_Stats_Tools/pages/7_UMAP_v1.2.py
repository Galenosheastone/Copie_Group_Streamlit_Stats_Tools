...
🛠 v1.2 (this) – SHAP beeswarm and violin plot sections fully robust against
                 DataFrame shape errors. Melt used for violin plot fallback.
"""
...
if do_shap:
    ...
    # Always force (n_samples, n_features) shape for plotting
    if isinstance(shap_vals, list):
        # (classes, N, F) -> (N, F)
        shap_beeswarm = np.mean(np.abs(np.stack(shap_vals, axis=0)), axis=0)
    else:
    ...
    with st.expander("Full SHAP beeswarm"):
        try:
            ...
            try:
                # robust long-form melt for violin
