...
# -*- coding: utf-8 -*-
"""
2_UMAP_Streamlit.py  ·  v1.9  (2025-05-28)

Fully robust Streamlit wrapper for the
“UMAP Metabolomics + SHAP Feature Importance” pipeline.
...
st.markdown(
    "Upload a **processed, wide-format metabolomics CSV**<br>"
...
def dl_btn(label, data, fname):
    st.download_button(label, data, fname, mime="text/csv")
...
def umap_embed(data, dims):
    return umap.UMAP(n_components=dims,
...
def train_xgb(Xa, ya):
    model = XGBClassifier(n_estimators=n_trees, random_state=random_state)
    model.fit(Xa, ya)
    return model
...
# ───────────── 5 · Static 2-D UMAP with ellipses ─────────────
...
for g in groups:
    ...
    ax2d.scatter(sel["UMAP1"], sel["UMAP2"], s=60, alpha=.7,
...
# ───────────── 6 · Interactive 3-D UMAP (Plotly) ─────────────
fig3d = go.Figure()
for g in groups:
    sel = emb3d[emb3d[group_col] == g]
    fig3d.add_trace(go.Scatter3d(
        x=sel["UMAP1"], y=sel["UMAP2"], z=sel["UMAP3"],
        mode="markers", name=g,
        marker=dict(size=4, color=colors[g], opacity=.75)
    ))
fig3d.update_layout(scene=dict(xaxis_title="UMAP1",
                               yaxis_title="UMAP2",
                               zaxis_title="UMAP3"),
...
# ─────────────────── 8 · XGBoost + SHAP (fully aligned) ───────────────────
model = train_xgb(Xs, y_enc)

if do_shap:
    explainer, raw_sv = shap_vals(model,
                                  pd.DataFrame(X, columns=X.columns))

    # Stack & trim to the real number of SHAP columns
    ...
    # Trim every SHAP matrix to n_shap_feat
    ...
    X_disp = X.iloc[:, :n_shap_feat].copy()
...
for i in np.unique(y_enc):
    ...
    axs[1,0].text(-.05, y0 + 0.5*len(seg), str(i));  y0 = y1 + 10
...
fig_val.savefig(os.path.join(DIRS["validation_plots"],
...
st.caption("© 2025 Galen O’Shea-Stone | Streamlit ≥ 1.33 • Python ≥ 3.9 • Script v1.9")
