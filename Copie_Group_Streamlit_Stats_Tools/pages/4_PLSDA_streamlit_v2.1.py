...
def optimize_components_cached(X_train: np.ndarray, y_train: np.ndarray, n_splits: int = 10) -> int:
    ...
    sizes = [len(idx) for idx, _ in kf.split(X_train)]
    allowed = max(min(sizes + [X_train.shape[1]]) - 1, 1)
    mean_r2 = []
...
@st.cache_data(show_spinner=False)
def build_3d_plot(pls_model, X_train: np.ndarray, y_train: np.ndarray,
...
def perform_permutation_test_with_visualization(model, X_train, y_train,
    ...
    if method == "accuracy":
        ...
        p_val = (np.sum(perm_acc >= orig_acc) + 1) / (n_permutations + 1)
        ...
        p_val = (np.sum(perm_d >= dist_actual) + 1) / (n_permutations + 1)
        fig, ax = plt.subplots(figsize=(8, 4))
