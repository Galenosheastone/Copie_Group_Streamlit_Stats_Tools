...
def compute_normality_metrics(
    ...
    for col in numeric_columns:
        ...
        if len(vals) > 3:
            ...
            skew_list.append(np.nan); kurt_list.append(np.nan)
            shapiro_ps.append(np.nan); ad_success.append(np.nan)
...
if st.button("Run Comparison"):
    ...
    for n in selected_norm:
        for l in selected_log:
            for s in selected_scale:
                ...
                row = {'Normalization':n, 'Log':l, 'Scaling':s,
    ...
    if not results_df.empty:
        ...
        if 'prop_shapiro>0.05' in ranked and 'avg_abs_skew' in ranked:
            ...
            for i in best.index:
                r = ranked.loc[i]
                ax.scatter(r['prop_shapiro>0.05'], r['avg_abs_skew'],
...
scale_choice = st.selectbox("Scaling method",
                            list(scale_map.keys()), index=2)
...
if st.button("Process Data"):
    ...
    # Heatmaps
    heat = st.selectbox("Heatmap Type",
