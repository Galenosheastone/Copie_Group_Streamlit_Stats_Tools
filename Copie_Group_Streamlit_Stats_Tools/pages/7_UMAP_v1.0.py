...
def plot_umap2d(df2d: pd.DataFrame) -> plt.Figure:
    ...
    for g in groups:
        ...
        ax.scatter(gdat["UMAP1"], gdat["UMAP2"], label=g,
                   color=color_map[g], alpha=.7, s=60)
        confidence_ellipse(gdat["UMAP1"], gdat["UMAP2"], ax,
                           facecolor=color_map[g], alpha=.15, edgecolor='black')
...
for g in groups:
    ...
    fig3d.add_trace(go.Scatter3d(
        x=gdat["UMAP1"], y=gdat["UMAP2"], z=gdat["UMAP3"],
        mode="markers", name=g,
...
X_tr, X_te, y_tr, y_te = train_test_split(
    X_scaled, y_enc, stratify=y_enc, test_size=.2, random_state=random_state
)
...
for i in np.unique(y_enc):
    ...
    axs[1, 0].fill_betweenx(np.arange(y_low, y_up), 0, ith, facecolor=color, alpha=.7)
    axs[1, 0].text(-0.05, y_low + .5 * len(ith), str(i))
    y_low = y_up + 10
