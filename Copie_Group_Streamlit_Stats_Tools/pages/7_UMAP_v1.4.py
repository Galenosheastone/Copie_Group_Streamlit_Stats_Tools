...
# -*- coding: utf-8 -*-
"""
2_UMAP_Streamlit.py (robust-fix 2025-05-28, v1.7)
...
🎯 v1.3 – convert arrays to lists before DataFrame construction
⚙️ v1.4 – corrected indentation in validation silhouette loop
🔧 v1.5 – replace pandas barh with matplotlib barh
🩹 v1.6 – fix multi-class SHAP broadcasting bug (stack vs list)
🩹 v1.7 – fix imbalanced-class “inhomogeneous shape” error in
          violin-plot fallback (use pre-stacked matrix)
...
if do_shap:
    ...
    # ------------------------------------------------------------------
    # Build (Σn_i × n_feat) matrix for the bar chart
    # Keep list-of-arrays for SHAP's beeswarm
    # ------------------------------------------------------------------
    ...
    st.pyplot(fig_bar)
    fig_bar.savefig(os.path.join(OUTPUT_DIRS["shap_plots"],
                                 "shap_bar.png"), dpi=600)
    ...
    with st.expander("Full SHAP beeswarm"):
        try:
            ...
            # Use the stacked matrix (Σn_i × n_feat) – always safe
            shap_mat = shap_for_bar
            df_v = pd.DataFrame(shap_mat, columns=X.columns)
...
st.caption("© 2025 Galen O’Shea-Stone • Streamlit ≥ 1.33 | Python ≥ 3.9 | Script v1.7")